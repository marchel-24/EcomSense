...
const SearchHistory: React.FC<SearchHistoryProps> = ({ setIsFocused, setInputValue }) => {
    ...
    return (
        ...
        >
            <div className={`flex flex-col space-y-2 overflow-y-auto transition-all duration-300 ${history.length > 3 ? "max-h-[160px]" : "max-h-full"}`}>
                {history.map((item, index) => (
                    <div
                        key={index}
                        className="flex justify-between items-center px-4 py-2 border-b border-gray-400 last:border-none"
                    >
